...
const { delimiter, extname, isAbsolute, resolve } = require("path");
const fs = require("fs");

export async function updatePythonPath(command) {
  const detect = quoteIfNecessary(
    "import sys; print(sys.hexversion >= 0x03060000)"
  );
  ...
  switch (process.platform) {
    ...
    default:
      console.error(
        "Python interpreter not found -- please install python >= 3.6"
      );
      break;
  }
